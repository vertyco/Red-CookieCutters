...
class DatabaseCommands(MixinMeta):
    @commands.group(name="dbsetgroup")
    @commands.is_owner()
    ...
    async def dbsetgroup_nukedb(self, ctx: commands.Context, confirm: bool):
        ...
        if not confirm:
            return await ctx.send(
                f"You must confirm this action with `{ctx.clean_prefix}dbsetgroup nukedb True`"
            )
        config = await self.bot.get_shared_api_tokens("postgres")
        if not config:
            return await ctx.send(
                f"Postgres credentials not set! Use `{ctx.clean_prefix}dbsetgroup postgres` command!"
            )
        ...
        await ctx.send("Database has been nuked and reinitialized, reload the cog.")
